...
部署配置集中在 `backend/`（`Dockerfile`、`compose.yaml`、`compose.prod.yaml`、`.env.example`）。**本手册中的 `docker compose` / `docker build` 命令默认在 `backend/` 目录下执行。**

Compose 读取 `backend/.env`（从 `backend/.env.example` 复制），至少填写：
...
- `WECHAT_APP_ID` 必须与 `miniprogram/project.config.json` 一致。
...
正式部署使用独立的 [compose.prod.yaml](../backend/compose.prod.yaml)，不启动仓库内 PostgreSQL/Redis，不提供弱默认值，并强制注入镜像、数据库、Redis、微信凭证。执行顺序：
