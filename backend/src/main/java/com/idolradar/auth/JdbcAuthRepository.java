...
public class JdbcAuthRepository implements AuthRepository {
    ...
    public UUID ensureUser(String openId) {
        ...
        jdbc.sql("INSERT INTO idr_user (openid) VALUES (:openId) ON CONFLICT (openid) DO NOTHING")
                .param("openId", openId)
                .update();
        return jdbc.sql("SELECT id FROM idr_user WHERE openid = :openId")
                .param("openId", openId)
                .query(UUID.class)
                .single();
    }
    ...
    public void createSession(UUID userId, String tokenHash, Instant expiresAt) {
        transactions.executeWithoutResult(status -> {
            ...
            jdbc.sql("DELETE FROM idr_user_session WHERE user_id = :userId AND (expires_at <= NOW() OR "
                            + "token_hash NOT IN ("
                            + "SELECT token_hash FROM idr_user_session WHERE user_id = :userId "
    ...
    public Optional<StoredIdentity> findSession(String tokenHash) {
        ...
        if (row.lastUsedAt().isBefore(Instant.now().minusSeconds(300))) {
            jdbc.sql("UPDATE idr_user_session SET last_used_at = NOW() WHERE token_hash = :tokenHash "
